...
K = 10
H = 0.7
print "Started MNIST Polynomial"
...
def run(polynomial_params_list, x_dataset, y_dataset, validation='hd'):
    """
    validation: hd for holdout and cv for cross validation k fold
    """
    classifiers = []
    for params in polynomial_params_list:
        classifiers.append(ClassifierWrapper(classifier=SVC, params=params))
        
    print "# of classifiers: {}".format(len(classifiers))
    results = polynomial_params_list
    for i, classifier in enumerate(classifiers):
        print '# {} with params: {}'.format(i, polynomial_params_list[i])    
        if validation == 'cv':
...
def main():
    ...
    polynomial_params_list = list(ParameterGrid(polynomial_params))
    results = run(polynomial_params_list, x_dataset, y_dataset)
    results.to_csv('results_polynomial_svm_200pcs.csv', index=False)
