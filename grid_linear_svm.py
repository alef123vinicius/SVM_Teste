...
def run(linear_params_list, x_dataset, y_dataset, validation='hd'):
    """
    ...
    results = linear_params_list
    for i, classifier in enumerate(classifiers):
        print '# {} with params: {}'.format(i, linear_params_list[i])    
...
def main():
    ...
    linear_params_list = list(ParameterGrid(linear_params))
    results = run(linear_params_list, x_dataset, y_dataset)
    results.to_csv('results_linear_svm_200pcs.csv', index=False)
